...
class testResponse(FrameProcessor):
    def __init__(self):
        super().__init__()
        self.name = "testResponse"

    async def process_frame(self, frame: Frame, direction: FrameDirection):
        try:
            await super().process_frame(frame, direction)
            if isinstance(frame, TextFrame):
                logger.info(f"Here is a text frame: {frame.text}")
            elif isinstance(frame, LLMMessagesFrame):
                logger.info(f"Here is a llmessage frame: {frame.messages}")
